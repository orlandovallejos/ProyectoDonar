...
public class CrearNecesidadResource {
    ...
    public Response postStrMsg( String nec) throws SQLException { //en com llega el json con todos los datos
        ...
        try{
            ...
            insert.insert("INSERT INTO necesidades ( titulo, necesidad,fecha_creacion,fecha_fin,telefono,facebook,twitter,usuarios_usuario,direccion,dia_horario,email,categorias_nombre_categoria,imagen_path,recaudacion_total,usuario_mp) VALUES (?,?,STR_TO_DATE( ?, '%Y-%m-%d'),STR_TO_DATE( ?, '%Y-%m-%d'),?,?,?,?,?,?,?,?,?,?,?)",datos);
